...
export function ProductCard({
  ...
  return (
    <motion.article
      ...
      className={`card-shell group rounded-[2rem] border border-ruby/8 bg-white/60 p-2.5 shadow-soft backdrop-blur-xl transition-shadow duration-500 hover:shadow-jewel ${className}`}
    >
      <div className="card-media relative aspect-[3/4] rounded-[1.65rem] bg-mist sm:aspect-[4/5]">
        <Image
          src={image}
          alt={title}
          fill
      ...
      <div className="card-body flex items-end justify-between gap-6 px-4 py-5">
        <div>
          <p className="caps-label caps-36 text-xs font-medium uppercase text-ruby/70">
            {eyebrow}
          </p>
          <h3 className="mt-2 font-display text-2xl leading-snug text-charcoal">{title}</h3>
        </div>
