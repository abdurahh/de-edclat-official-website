...
export function ContactForm() {
  ...
  const isOthers = selectedService === "Others";

  function resolvedSubject() {
    if (!isOthers) return selectedService;
    const custom = otherSubject.trim();
    return custom ? `Others: ${custom}` : "";
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!subject) {
      setStatus("error");
      setErrorMessage("Please describe your subject of interest.");
      return;
    }
      ...
      >
        ...
        <div className="mt-5">
          ...
          <div className="relative mt-3">
            <input name="subject" type="hidden" value={resolvedSubject()} />
            <motion.button
              type="button"
              whileTap={{ scale: 0.99 }}
                  ...
                  {services.map((service) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (service !== "Others") setOtherSubject("");
                        setIsOpen(false);
                      }}
